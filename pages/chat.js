    ...
    models.user.findOne({ 'email': req.session.name }, function (err, doc) {
      models.user.findOne({ '_id': req.body.id }, function (err, chatter) {
        ...
        if (req.body.sendMsg == 'sendMessage') {
          ...
          models.user.findOneAndUpdate({ 'email': req.session.name }, { $push: { 'contacts': chatter.email }}, function (err, contacts) {
            if (err) {
          ...
          models.user.findOneAndUpdate({ '_id': req.body.id }, { $push: { 'contacts': doc.email }}, function (err, temp) {
            if (err) {
